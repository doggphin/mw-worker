...
impl PhotoGroupOptions {
    pub fn is_finished(expecting_media: ExpectingMedia, counted_media : ExpectingMedia) -> Result<(), FCError> {
        ...
        if let Some(expecting) = expecting_media.slides {
            let counted = match counted_media.slides {
                Some(v) => v,
        ...
        if let Some(expecting) = expecting_media.prints {
            let counted = match counted_media.prints {
                Some(v) => v,
        ...
        if let Some(expecting) = expecting_media.negatives {
            let counted = match counted_media.negatives {
                Some(v) => v,
